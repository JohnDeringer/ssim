    ...
    public Set<EncounterResponse> getEncountersBySearch(SearchType search) {
        ...
        for (Encounter encounter : encounterDao.findEncounters(search, solrIds)) {
            ...
            for (Artifact artifact : encounter.getArtifacts()) {
                // Get the files for this encounter
                for (ArtifactFile file : artifact.getArtifactFiles()) {
                    ...
                    if (fileType != null) {
                        try {
                            FormatType formatType =
                                    FormatType.fromValue(
                                        fileType.getValue()
                                    );
                            fileResponse.setFormat(formatType);
                        } catch (Exception e) {
                            logger.error("Error transforming String to Enum [" + fileType.getValue() + "]");
                        }
                    }
                    // Add files to files list
                    encounterResponse.getFiles().add(fileResponse);
                }
